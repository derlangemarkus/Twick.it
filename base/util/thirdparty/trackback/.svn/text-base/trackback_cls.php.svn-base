...
class Trackback {
	...
	function negotiate($url) {
		...
		$host = $urlInfo["host"];
		$port = getArrayElement($urlInfo, "port", 80);
		if (!is_numeric($target["port"])) {
            $port = 80;
        } 
		$query = getArrayElement($urlInfo, "query", "");
		$path = getArrayElement($urlInfo, "path", "") . ($query ? "?".$query : "");
        ...
        if (preg_match('/<link\s+rel="pingback"\s+href="([^"]+)"/si', $response, $pingpack)) {
            return $pingpack[1];
        }
	...
	function createXml($inSource, $inTarget) {
		// http://www.martins-kabuff.de/blog/?id=464f135b6780f
		return "
			  ...
			  <value>
				<struct>
				  <member>
					<name>sourceURI</name>
					<value>
					  <string>$inSource</string>
					</value>
				  </member>
				  <member>
					<name>targetURI</name>
					<value>
					  <string>$inTarget</string>
					</value>
				  </member>
				</struct>
			  </value>
    ...
    function createWordpressXml($inSource, $inTarget) {
		// http://www.martins-kabuff.de/blog/?id=464f135b6780f
		return "
		<?xml version='1.0' encoding='utf-8'?>
        <methodCall>
     ...
     * @return boolean 
     */
    function ping($targetUrl, $url)
    {		
		$pingbackUrl = self::negotiate($targetUrl);
		if(!$pingbackUrl) {
            return false;
        }
        ...
        foreach(array(self::createXml($url, $targetUrl), self::createWordpressXml($url, $targetUrl)) as $xml) {
            $socket = fsockopen($target["host"], $target["port"]);

            if (is_resource($socket)) {
                fputs($socket, "POST " . $target["path"] . $target["query"] . " HTTP/1.1\r\n");
                fputs($socket, "Host: " . $target["host"] . "\r\n");
                ...
                echo($xml);
                echo("<hr />");
                print_rr(htmlspecialchars($response));
                echo("<hr />");echo("<hr />");echo("<hr />");
                */

                $success = strpos($response, '<error>0</error>');
                if($success) {
                    return true;
                }
            } else {
